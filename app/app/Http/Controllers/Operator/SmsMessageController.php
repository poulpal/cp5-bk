    ...
    public function index(Builder $builder)
    {
        if (request()->ajax()) {
            return DataTables::of(
                SmsMessage::query()->orderBy('status', 'asc')->orderBy('created_at', 'desc')
            )
                ...
                ->addColumn('action', function ($item) {
                    ...
                    return '<a href="' . route('operator.smsMessages.accept', $item->id) . '" class="btn btn-sm btn-success">تایید و ارسال</a>';
                })
        ...
        return view('operator.smsMessages', compact('table'));
    }

    public function accept(SmsMessage $smsMessage)
    {
        ...
        if ($credit < ($smsPrice * 10 * $smsMessage->count * $smsMessage->length)) {
            session()->flash('error', 'اعتبار پیامک کافی نیست.');
            return redirect()->route('operator.smsMessages.index');
        }
