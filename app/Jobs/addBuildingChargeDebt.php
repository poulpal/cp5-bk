...
class addBuildingChargeDebt
{
    ...
    public function handle()
    {
        $buildings = Building::whereHas('options', function ($query) {
            $query->where('auto_add_monthly_charge', true)->where('charge_day', Jalalian::now()->getDay());
        })->cursor();
        foreach ($buildings as $building) {
            ...
            foreach ($units as $unit) {
                ...
                if ($unit->charge_fee == 0) {
                    continue;
                }
                $charge_debt = $unit->charge_debt;
                $charge_debt += $charge_fee;
                ...
                $invoice = Invoice::create([
                    ...
                    'status' => 'paid',
                    'payment_method' => 'cash',
                    'description' => 'هزینه شارژ ' . Jalalian::now()->format('F Y'),
                    'serviceable_id' => $unit->id,
                    'serviceable_type' => BuildingUnit::class,
                    'is_verified' => 1,
                    'debt_type_id' => $building->debtTypes->where('name', 'شارژ جاری')->first()->id ?? $building->debtTypes->first()->id,
                    'early_discount_until' => $building->options->early_payment ? Carbon::now()->addDays($building->options->early_payment_days)->endOfDay() : null,
                    'early_discount_amount' => $building->options->early_payment ? $building->options->early_payment_percent * $charge_fee / 100 : null,
                ]);
                ...
                if (!$paidWithWallet && $unit->charge_debt > 0) {
                    ...
                    $resident->notify(
                        (new ChargeAddedNotfication($charge_fee, $unit->charge_debt, $unit->token, 'شارژ ' . Jalalian::now()->format('F')))
                            ->delay($send_time)
                    );
                }
            }
